...
# --- FUNÇÃO PARA EXPORTAÇÃO VISUAL (HTML) ---
def gerar_html_exportacao(dados):
    linhas = ""
    for p in dados:
        linhas += f"""
        <tr>
            <td style='padding:10px; border-bottom:1px solid #eee;'>{p['Produto']}</td>
            <td style='padding:10px; border-bottom:1px solid #eee;'>R$ {p['Preço ML']:.2f}</td>
            <td style='padding:10px; border-bottom:1px solid #eee; color:#10B981;'>R$ {p['Lucro ML']:.2f}</td>
            <td style='padding:10px; border-bottom:1px solid #eee;'>R$ {p['Preço Shopee']:.2f}</td>
            <td style='padding:10px; border-bottom:1px solid #eee; color:#10B981;'>R$ {p['Lucro Shopee']:.2f}</td>
        </tr>
        """
    ...
    <body style='font-family: Arial, sans-serif; padding: 20px;'>
        <h2 style='color: #764ba2;'>Relatório Precificador Pro</h2>
        <table style='width: 100%; border-collapse: collapse;'>
            <thead>
                <tr style='background: #f8fafc; text-align: left;'>
                    <th style='padding:10px; border-bottom: 2px solid #eee;'>Produto</th>
                    <th style='padding:10px; border-bottom: 2px solid #eee;'>Preço ML</th>
                    <th style='padding:10px; border-bottom: 2px solid #eee;'>Lucro ML</th>
                    <th style='padding:10px; border-bottom: 2px solid #eee;'>Preço SHP</th>
                    <th style='padding:10px; border-bottom: 2px solid #eee;'>Lucro SHP</th>
                </tr>
            </thead>
            <tbody>{linhas}</tbody>
        </table>
...
# 2. TABELA DE HISTÓRICO E COMPARATIVO
if st.session_state.db:
    ...
    with st.container():
        ...
        relatorio_html = gerar_html_exportacao(st.session_state.db)
        st.download_button(
            label="📥 EXPORTAR RELATÓRIO VISUAL (HTML)",
            data=relatorio_html,
            file_name="relatorio_precificacao.html",
            mime="text/html",
            use_container_width=True
        )
    st.markdown('</div>', unsafe_allow_html=True)
